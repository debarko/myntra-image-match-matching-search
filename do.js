...
var color_match =
{
	"Red": ["Beige", "Lime", "Chocolate", "Black", "White"],
	"Lime": ["Red", "Grey", "Black"],
	"Chocolate": ["Red", "Navy blue", "White", "Black", "Dark Green"],
	"Salmon": ["Red"],
	"Yellow": ["Red", "Navy blue", "Sky blue", "Black"],
	"Blue": ["White", "Black", "Dark Green"],
	"Sky blue": ["Yellow", "Chocolate", "Dark Green", "Pink"],
	"Black": ["Black"],
	"Green": ["Navy blue", "Black", "Beige", "Yellow"],
	"Navy blue": ["Yellow", "Green", "Sea Green"],
	"Peach": ["Red", "Black", "Brown"],
	"Pink": ["Black", "Grey", "Dark Green"],
	"Ash grey": ["Black", "White", "Dark Green"],
...
function setup_data(points, callback) {
	var data = [];
	for (var i = 0; i < points.length; ++i) {
		data.push(new Point(points[i]["x"], points[i]["y"], points[i]["z"], points[i]["label"]));
	}
...
ColorClassifier.prototype = {
	...
	classify_rgb: function (rgbPoint) {
		var min = Infinity;
		var min_idx = -1;
		var i, dist;
		for (i = 0; i < this.data.length; ++i)
		{
			dist = rgbPoint.dist(this.data[i]);
			if (dist < min)
			{
				min = dist;
				min_idx = i;
...
var file, classy = new ColorClassifier();
var classy2 = new ColorClassifier();
var spawn = require( 'child_process' ).spawn, final_json_dump = [], count = 0,
server_side_response = 0,
j2p, shape, internal_color_of_image, type_of_image, external_color_of_image;
...
setup_data(orig_data,
	function(data) {
		classy.learn(data);
	});

setup_data(orig_data2,
	function(data) {
		classy2.learn(data);
	});

function process_Req(val) {
	console.log(val);
	var request = http.get(val, function(response) {
		var parsed = url.parse(val);
		file = fs.createWriteStream('./download/'+path.basename(parsed.pathname));
		response.pipe(file);
		j2p = spawn( './j2p', [ './download/'+path.basename(parsed.pathname) ] )
		j2p.stdout.on( 'data', ( data ) => {
			console.log( `stdout: ${data}` );
		});

		j2p.stderr.on( 'data', ( data ) => {
			console.log( `stderr: ${data}` );
		});
		j2p.on( 'close', ( code ) => {
			if (code === 0) {
				shape = spawn( './shape', [ 'q.png', '12'] );
				shape.stdout.on( 'data', ( data ) => {
					data = `${data}`;
					data = data.split('\n');
					type_of_image = data[0];
					external_color_of_image = classy.classify_rgb(new Point(data[1].split(',')[2], data[1].split(',')[1], data[1].split(',')[0]));
					internal_color_of_image = classy2.classify_rgb(new Point(data[1].split(',')[2], data[1].split(',')[1], data[1].split(',')[0]));
						//console.log(internal_color_of_image, 'External Color: ' , external_color_of_image, type_of_image);
						process_colors();
					});
			}
		});
...
function process_colors() {
	var colors = color_match[internal_color_of_image];
	type_of_image = (type_of_image === 'shirt') ? 'pant' : 'shirt';
	for (var i = 0; i < colors.length; i++) {
		getProducts(colors[i], type_of_image);
	}
};



function getProducts(color, type) {
	var url = 'http://developer.myntra.com/search/data/';
	var search_term = color + ' ' + type;
	search_term = search_term.split(' ').join('_');

	request(url + search_term, function(error, response, body) {
		body = JSON.parse(body);
		var prods = body.data.results.products;
		if (prods.length > 5) {
			prods.length = 5;
		}
